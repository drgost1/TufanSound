...
void FxController::init(FxMainWindow* main_window, FxSystemTrayView* system_tray_view, AudioPassthru* audio_passthru)
{
	if (!isTimerRunning())
	{
        ...
        {
            String message(TRANS("Error in system audio configuration. Unable to run TUFAN"));
        ...
        if (prev_version != app_version)
        {
			...
			{
				FxMessage::showMessage(TRANS("TUFAN - powered by open-source FxSound"), { TRANS("GitHub"), "https://github.com/drgost1/TufanSound" });
			}
            view_ = ViewType::Pro;
...
void FxController::hideMainWindow()
{
	...
	{
        Thread::sleep(2000);
		FxModel::getModel().pushMessage(TRANS("TUFAN in system tray\r\nClick TUFAN icon to reopen"));
		minimize_tip_ = false;
	}
}
        ...
        if (survey_tip_)
        {
            ...
            else
            {
                ...
                if (current_time > survey_timer)
                {
                    ...
                    String message = TRANS("Thanks for using TUFAN! Would you be\r\ninterested in helping us by taking a quick 4 minute\r\nsurvey so we can make TUFAN better?");
                    if (authenticated_)
                    {
	...
	switch (message)
	{
		case WM_HOTKEY:
		{
			if (w_param == CMD_ON_OFF)
			{
                ...
                FxModel::getModel().pushMessage(controller->FormatString(TRANS("TUFAN is %s."), param));
			}
